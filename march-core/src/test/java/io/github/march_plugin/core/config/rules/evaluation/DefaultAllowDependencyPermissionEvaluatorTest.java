...
class DefaultAllowDependencyPermissionEvaluatorTest {
    ...
    private final DefaultAllowDependencyPermissionEvaluator evaluator = new DefaultAllowDependencyPermissionEvaluator(projectStructureRoot);

    private RuleRegistry registryOf(final Rule... rules) {
        final var builder = new RuleRegistry.Builder();
        builder.setRuleStrategy(RuleStrategy.DEFAULT_ALLOW);
    ...
    @Test
    void forbidsWhenARuleReducesToAlwaysTrue() {
        final var rule = new Rule("Different domain forbidden", differentDomainRule, Rule.RuleScope.GLOBAL);
    ...
    void ignoresModuleOnlyRules() {
        final var moduleOnlyRule = new Rule("Different domain forbidden", differentDomainRule, Rule.RuleScope.MODULE_ONLY);
